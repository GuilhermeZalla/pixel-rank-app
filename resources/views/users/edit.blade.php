<x-layout>
    ...
    <div class="flex flex-row justify-between gap-8 mt-[10vh] mx-[2vw]" x-data="{ activeMenu: '{{ $menu }}' }">
        <section class="flex flex-col w-100 border border-[#88888833] bg-[#0E0E0E] rounded-[10px] py-6 h-130 gap-6">
            ...
            <ul class="flex flex-col gap-4 w-full p-0">
                <x-nav-link-edit x-on:click="activeMenu = 'menu1'" x-bind:class="{ 'border-accent' : activeMenu === 'menu1', 'border-transparent': activeMenu !== 'menu1' }">Profile</x-nav-link-edit>
                <x-nav-link-edit x-on:click="activeMenu = 'menu2'" x-bind:class="{ 'border-accent' : activeMenu === 'menu2', 'border-transparent': activeMenu !== 'menu2'}">My Reviews</x-nav-link-edit>
                <x-nav-link-edit x-on:click="activeMenu = 'menu3'" x-bind:class="{ 'border-accent' : activeMenu === 'menu3', 'border-transparent': activeMenu !== 'menu3' }">My Comments</x-nav-link-edit>
                <x-nav-link-edit x-on:click="activeMenu = 'menu4'" x-bind:class="{ 'border-accent' : activeMenu === 'menu4', 'border-transparent': activeMenu !== 'menu4' }">Settings</x-nav-link-edit>
            </ul>
        </section>
        <section class="flex flex-col gap-5 w-full border border-[#88888833] bg-[#0E0E0E] rounded-[10px] p-6">
            ...
            <x-form.form method="POST" action="{{ route('users.edit', auth()->user()->id) }}" class="w-full" x-show="activeMenu === 'menu1'">
                ...
                <x-form.field>
                    ...
                    <x-form.input type="text" name="name" id="name" placeholder="This nickname will be displayed publicly on the site"
                        required />
                </x-form.field>
                <x-form.field>
                ...
                <x-form.field class="mt-2 flex justify-end">
