<x-layout>
    ...
    <div class="px-4 sm:px-0 md:px-40 lg:px-20">
            ...
            <nav class="flex flex-row justify-between">
                ...
                <div class="flex flex-row gap-3">
                    @if (!empty($review))
                        ...
                        @endcan
                        @can('delete', $review)
                            <x-badge-link for="my_modal_7" class="text-[#FF6764] cursor-pointer btn"><x-heroicon-s-trash class="size-3.5" />Delete</x-badge-link>
                        @endcan
                    @endif
                </div>
            </nav>
        ...
        <x-section class="flex flex-col">
            <div class="max-h-90 w-full overflow-hidden rounded-tr-xl rounded-tl-xl relative">
                <img src="https://images.igdb.com/igdb/image/upload/t_1080p/{{ $game['cover'] }}.jpg" alt="{{ $review->game->title . ' cover' }}" class="w-full h-full object-contain rounded-tr-xl rounded-tl-xl">
            ...
            <div class="py-8 px-3 flex flex-col gap-3 -mt-30 relative z-10">
                ...
                <div class="flex flex-col gap-6">
                    <ul class="flex flex-row gap-5">
                        <li>
                            <x-badge-link href="/{{ $review->recommendation->label() }}" type="link" class="rounded-[10px] px-1 py-0">{{ $review->recommendation->label() }}</x-badge-link>
                        </li>
                        ...
                        <li id="comment-{{ $comment->id }}" class="flex flex-row gap-4 border-b border-white/8 py-8">
                            ...
                            <div class="w-full">
                                <span class="flex flex-row justify-between">
                                    <h3 class="font-bold h-0">{{ $comment->user->name }}</h3>
